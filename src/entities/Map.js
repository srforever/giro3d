...
function subdivideNode(context, map, node) {
    if (!node.children.some(n => n.layer === map)) {
        ...
        for (const extent of extents) {
            ...
            // HERE never elevationLayers and colorLayers ???
...
class Map extends Entity3D {
        ...
        if (node.visible) {
            ...
            if (!this.frozen) {
                ...
                if (this.testTileSSE(node, sse)
                    ...
                    // display if children aren't ready
                    node.setDisplayed(false);
                    requestChildrenUpdate = true;
                } else {
                    node.setDisplayed(true);
                    if (this.smart && !node.userData.smart) {
                        node.visible = false;
                    }
                }
            } else {
    ...
    addLayer(layer) {
        return new Promise((resolve, reject) => {
            ...
            if (layer.protocol === 'cog' && layer instanceof ElevationLayer) {
                this.smart = true;
            };
            ...
            layer.whenReady.then(l => {
                this.attach(l);
                if (l.useAsObject) {
                    this._instance.threeObjects.add(l.object3d);
                }
                // Small fix for nicer controls
                if (this.smart && l instanceof ElevationLayer) {
                    this._instance.controls.target.setZ(l.minmax.min);
                }
