...
export default {
     ...
     * @param _pointAttributes the point attributes information contained in
     * layer.metadata coming from cloud.js
     * @returns a promise that resolves with a BufferGeometry.
     */
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    parse: function parse(buffer: ArrayBuffer, _pointAttributes: string[]) {
        if (!buffer) {
            throw new Error('No array buffer provided.');
        }

        // Format:
        // MinX,MinY,MinZ,MaxX,MaxY,MaxZ,X1,Y1,Z1,[...],XN,YN,ZN,R1,G1,B1,A1,[...],RN,GN,BN,AN
        const view = new DataView(buffer, 0, 6 * 4);
        const min = new Vector3(
            view.getFloat32(0, true), view.getFloat32(4, true), view.getFloat32(8, true),
        );
        const max = new Vector3(
            view.getFloat32(12, true), view.getFloat32(16, true), view.getFloat32(20, true),
        );
