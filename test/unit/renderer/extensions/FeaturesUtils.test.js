...
describe('FeaturesUtils', () => {
    it('should correctly parse geojson', () => promise.then(collection => {
        assert.equal(collection.features.length, 3);
    }));
    it('should correctly compute extent geojson', () => promise.then(collection => {
    ...
    it('should correctly filter point', () => promise.then(collection => {
    ...
    it('should correctly filter polygon', () => promise.then(feature => {
    ...
    it('should correctly filter line', () => promise.then(feature => {
        const coordinates = new Coordinates('EPSG:4326', 2.23, 43.39);
        const filter = FeaturesUtils.filterFeaturesUnderCoordinate(coordinates, feature, 0.1);
        assert.equal(filter.length, 1.0);
        assert.equal(filter[0].feature.type === 'linestring', 1.0);
    }));
    it('should remember individual feature properties', () => promise.then(feature => {
